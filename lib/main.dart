...
class _PlayerEntryState extends State<PlayerEntry> {
  ...
  void _syncAudioVideo() async {
    if (_videoController.value.isBuffering && !_isbufferingVideo) {
      _isbufferingVideo = true;
      await _audioController.pause();
    } else if (!_videoController.value.isBuffering && _isbufferingVideo) {
      _isbufferingVideo = false;
      await _audioController.play();
    }
    if (_audioController.processingState == ProcessingState.buffering &&
        !_isbufferingAudio) {
      _isbufferingAudio = true;
      await _videoController.pause();
    } else if (_audioController.processingState != ProcessingState.buffering &&
        _isbufferingAudio) {
  ...
  //   }
  //   if(_audioController.playerState == PlayerState.BUFFERING){
  //     await _videoController.pause();
  //   } else if()
  // }
...
class _AVControlsState extends State<AVControls> {
  ...
  void _syncAudioVideo() async {
    if (widget.videoController.value.isBuffering && !_isbufferingVideo) {
      _isbufferingVideo = true;
      await widget.audioController.pause();
    } else if (!widget.videoController.value.isBuffering && _isbufferingVideo) {
      _isbufferingVideo = false;
      await widget.audioController.play();
    }
    if (widget.audioController.processingState == ProcessingState.buffering &&
        !_isbufferingAudio) {
      _isbufferingAudio = true;
      await widget.videoController.pause();
    } else if (widget.audioController.processingState !=
